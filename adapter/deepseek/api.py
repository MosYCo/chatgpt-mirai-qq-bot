...
class DeepSeekAPIAdapter(BotAdapter):
    ...
    async def request(self, session_id: str = None, messages: list = None) -> str:
        api_endpoint, headers, data = self._prepare_request(session_id, messages, stream=False)
    ...
    async def request_with_stream(self, session_id: str = None, messages: list = None) -> AsyncGenerator[str, None]:
        api_endpoint, headers, data = self._prepare_request(session_id, messages, stream=True)
    ...
    async def ask(self, prompt: str) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            logger.debug(f"[尝试使用DeepSeek-API:{self.bot.engine}] 请求：{prompt}")
            ...
            if config.openai.gpt_params.stream:
                ...
                token_count = self.bot.count_tokens(self.session_id, self.bot.engine)
                logger.debug(f"[DeepSeek-API:{self.bot.engine}] 响应：{full_response}")
                logger.debug(f"[DeepSeek-API:{self.bot.engine}] 使用 token 数：{token_count}")
            else:
                yield await self.request(session_id=self.session_id)
            event_time = time.time() - start_time
            if event_time is not None:
                logger.debug(f"[DeepSeek-API:{self.bot.engine}] 接收到全部消息花费了{event_time:.2f}秒")

        except Exception as e:
            logger.error(f"[DeepSeek-API:{self.bot.engine}] 请求失败：\n{e}")
            yield f"发生错误: \n{e}"
            raise
